...
fn on_draw(frame: &mut TerminalFrame, app: &mut App) {
    ...
    frame.render_widget(widget::help_panel(), chunks[3]);
    frame.render_widget(widget::status_bar(app), chunks[4]);

    if let AppState::Adding = app.state {
        // popup需要先clear一下,否则下面的背景色会透上来
        frame.render_widget(widgets::Clear, chunks[5]);
        frame.render_widget(widget::stock_input(app), chunks[5]);

        // 显示光标, width()接口依赖一个外部包,可以正确处理中文宽度
        frame.set_cursor(chunks[5].x + app.input.width() as u16 + 1, chunks[5].y + 1);
    }
}
